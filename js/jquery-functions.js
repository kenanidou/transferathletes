$("document").ready(function () {
    ...
    function retrieveAnswers() {
        // Εμφάνιση όλων των δικαιολογητικών
        getEvidencesById(1);
        getEvidencesById(2);
        getEvidencesById(3);
        getEvidencesById(4);
    ...
    function submitForm() {
        ...
        const negativeSteps = [4, 6, 8, 10];
        const ministryStep = 2;

        // Έλεγχος καταλήξεων
        let isPositive = positiveSteps.every(q => allAnswers[q] === "Ναι") &&
                         negativeSteps.every(q => allAnswers[q] === "Όχι");

        let isNegative = negativeSteps.every(q => allAnswers[q] === "Ναι") &&
                         positiveSteps.every(q => allAnswers[q] === "Όχι");

        let isMinistry = allAnswers[ministryStep] === "Ναι";

        let finalResult = "";
        if (isPositive) {
            finalResult = "Θετική εισήγηση μεταγραφής";
        } else if (isNegative) {
            finalResult = "Αρνητική εισήγηση μεταγραφής";
        } else if (isMinistry) {
            finalResult = "Αποστολή αιτήματος προς Υπουργείο Παιδείας";
        } else {
            finalResult = "Δεν υπάρχει σαφές αποτέλεσμα (ανάμικτες απαντήσεις)";
        }
        ...
        $(".question-container").append("<h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε είναι:</h5><br />");
